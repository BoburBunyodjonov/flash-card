...
- **`GET /api/my-words/study/questions`** — Multi-method "memorize" session over the user's personal words. Query `mode` (`flashcard|mcq|reverse|typing|listening|scramble|cloze|matching|mixed`, default `mixed`), `count` (1–20, default 10; `matching` capped ~6). → `{ data: StudyQuestion[] }` (`[]` if no non-mastered words). **StudyQuestion:** `{ id (userWord id), mode (resolved single mode — never mixed; cloze may fall back to mcq; matching items all share mode "matching"), word, translation, pronunciation, audioUrl, prompt (word/translation/cloze-sentence/""), ttsWord?, choices?: string[4], correctIndex?, answer? (typing/scramble), scrambled? }`. `mixed` picks a method per word by SM-2 strength (recognition→recall→production).
- **`POST /api/my-words/study/submit`** — Body `{ answers: [ { id: uuid, correct: boolean } ] }` (1–50). Applies graded SM-2 to each personal word + capped XP (does NOT feed leagues). → `{ data: { xpEarned, correct, total } }`.
