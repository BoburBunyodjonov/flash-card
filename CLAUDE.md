...
- Optional **auto-transcript**: `services/transcription.service.ts` — Whisper STT (OpenAI-compatible, default Groq) generates transcript + segment timestamps (+ uz translation) so the admin doesn't hand-type. `TRANSCRIBE_API_KEY` in `.env`; admin clicks "Avto-transkript" in the import dialog
